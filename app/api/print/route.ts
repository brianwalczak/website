...
export async function POST(req: NextRequest) {
	...
	if (process.env.PRINT_ENABLED != "true") {
		return NextResponse.json({ error: "Sorry, printing is currently paused!" }, { status: 503 });
	}

	// Check if the request body is valid
	if (!body || typeof body.message !== "string") {
		return NextResponse.json({ error: "Your request is invalid." }, { status: 400 });
	}
	...
	if (!message || message.length === 0) {
		return NextResponse.json({ error: "You must enter a message, silly!" }, { status: 400 });
	}

	// Check if the message is too long
	if (message.length > MAX_PRINTER_CHARS) {
		return NextResponse.json({ error: `Message is too long! Max ${MAX_PRINTER_CHARS} characters.` }, { status: 400 });
	}

	// Check if the message has some inappropriate content
	if (matcher.hasMatch(message)) {
		return NextResponse.json({ error: "Sorry, your message was flagged for inappropriate content and cannot be printed." }, { status: 400 });
	}
	...
	if (process.env.PRINT_VPN_CHECK != "false" && process.env.PRINT_VPN_CHECK_API_KEY) {
		try {
			...
			return NextResponse.json({ error: "An unknown error occurred while checking your IP address. Please try again later." }, { status: 500 });
		}
	}
	...
	if (!success) {
		return NextResponse.json({ error: "You've already sent a message to the printer in the last 24 hours. Please wait a bit before sending another!" }, { status: 429 });
	}
