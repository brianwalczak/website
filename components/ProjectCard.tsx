...
export default function ProjectCard({ project }: { project: Project }) {
	...
	return (
		<a href={project.url} target="_blank" rel="noopener noreferrer" className="group border border-surface-border bg-surface rounded-xl overflow-hidden flex flex-col hover:bg-surface-hover hover:border-surface-border-hover transition-colors">
			<div className="relative w-full aspect-video overflow-hidden bg-backdrop">
				{image?.url && (
					<Image src={image.url} alt={image.alt ?? project.name} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover transition-transform duration-300 group-hover:scale-[1.03]" />
				)}
			</div>
